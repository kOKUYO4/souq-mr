...
const dealOfDay = listings.filter((l) => l.originalPrice && l.originalPrice > l.price)[0];
const flashDeals = listings.filter((l) => l.originalPrice && l.originalPrice > l.price).slice(0, 6);
...
export default function PromotionsPage() {
  ...
  const dealDiscount = dealOfDay.originalPrice
    ? Math.round(((dealOfDay.originalPrice - dealOfDay.price) / dealOfDay.originalPrice) * 100)
    : 0;
            ...
            <div className={`flex-1 flex flex-col justify-center ${isRTL ? "text-right" : ""}`}>
              ...
              <div className={`flex items-center gap-3 mb-5 ${isRTL ? "flex-row-reverse" : ""}`}>
                ...
                {dealOfDay.originalPrice && (
                  <span className="text-base text-night-400/40 line-through">{formatPrice(dealOfDay.originalPrice)} MRU</span>
                )}
              </div>
            ...
            {clearanceItems.map((l) => (
              <Link key={l.id} href={`/annonce/${l.id}`}
                ...
                <div className="p-3">
                  ...
                  {l.originalPrice && (
                    <p className="text-xs text-night-400/40 line-through">{formatPrice(l.originalPrice)} MRU</p>
                  )}
                </div>
              </Link>
            ))}
